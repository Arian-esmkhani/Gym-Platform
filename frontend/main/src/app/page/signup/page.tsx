...
import { useAuth } from "@/app/hooks/use-auch";
import { useState } from "react";
import { SaveUserDto } from "@/app/types/auch";

export default function Signup() {
    const [formData, setFormData] = useState<SaveUserDto>({
        username: '',
        password: '',
        email: 'test.test@gmail.com'
    });

    const { register, loading: hookLoading } = useAuth();

        const handleChange = (value: string, field: keyof SaveUserDto): void => {
        setFormData(prev => ({
            ...prev,
            [field]: value
        }));
        ...
        transition-all duration-500">
            <div className="p-14 flex flex-col">
                <div className="ml-13 sm:ml-19 text-black/50">
                    <span className="p-1 font-medium">signup
                ...
                <span className="pt-8 pl-3">Have accent?<Link href="/page/login" className="text-cyan-900">  login!</Link></span>
            </div>
        </div>
    )
}